...
import lim.jiyeon.hb88chnhthc.databinding.FragmentSecondBinding
...
class SecondFragment : Fragment() {
    ...
    private fun Jumpcodeeee() {
        if (code == true) {
            webView?.loadUrl(urlview.toString())
            Log.e("URLVIEW", urlview.toString())
        } else {
            if (title == "webview") {
                webView?.loadUrl("file:///android_asset/hbone.html")
            } else {
                webView?.loadUrl("file:///android_asset/hbtwo.html")
            }
        }
        init()
    }
